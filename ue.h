...
inline void DrawString(float fontSize, int x, int y, ImColor color, bool bCenter, bool stroke, const char* pText, ...)
{
	...
	if (stroke)
	{
		float outlineSize = 1.5f; // Adjust the outline size as needed
		ImVec4 outlineColor(0, 0, 0, 1); // Outline color (black in this case)

		// Draw text with outline
		ImGui::GetBackgroundDrawList()->AddText(ImGui::GetFont(), fontSize, ImVec2(x - outlineSize, y), ImGui::ColorConvertFloat4ToU32(outlineColor), text.c_str());
		ImGui::GetBackgroundDrawList()->AddText(ImGui::GetFont(), fontSize, ImVec2(x + outlineSize, y), ImGui::ColorConvertFloat4ToU32(outlineColor), text.c_str());
		ImGui::GetBackgroundDrawList()->AddText(ImGui::GetFont(), fontSize, ImVec2(x, y - outlineSize), ImGui::ColorConvertFloat4ToU32(outlineColor), text.c_str());
		ImGui::GetBackgroundDrawList()->AddText(ImGui::GetFont(), fontSize, ImVec2(x, y + outlineSize), ImGui::ColorConvertFloat4ToU32(outlineColor), text.c_str());
	}
...
inline void DrawFilledRect(int x, int y, int w, int h, ImU32 color, bool round = false)
{
	...
	if (round)
		ImGui::GetForegroundDrawList()->AddRectFilled(ImVec2(x, y), ImVec2(x + w, y + h), color, 30);
	else
